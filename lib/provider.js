...
class Provider {
  ...
  getSuggestions({editor, bufferPosition}) {
    ...
    if (pxReg.test(prefix)) {
      if (helper.isUseVwVh) {
        return this.getConvertSuggestionsForVwVh(prefix);
      }
      else {
  ...
  getConvertSuggestionsForVwVh(prefix) {
    const strVw = helper.getRem(prefix, 'vw');
    const strVh = helper.getRem(prefix, 'vh');
